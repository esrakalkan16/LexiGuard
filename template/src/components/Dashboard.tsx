...
const StatCard = ({ icon: Icon, label, value, color }: { icon: any, label: string, value: string, color: string }) => (
  <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm transition-hover hover:border-slate-300">
    <div className="flex items-center gap-4">
      <div className={cn(
        "p-2.5 rounded-lg",
        color === "blue" ? "bg-slate-50 text-slate-900" :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-xl">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Recent Investigations</span>
            <button className="text-[10px] font-bold text-blue-600 hover:underline uppercase tracking-widest">View All</button>
          </div>
          <div className="divide-y divide-slate-100">
            {[
              { name: "SaaS_Agreement_Alpha.pdf", type: "SaaS", status: "High Risk", date: "2h ago", score: 82 },
              { name: "Service_Protocol_v2.docx", type: "Service", status: "Low Risk", date: "Yesterday", score: 12 },
              { name: "NDA_Final.txt", type: "NDA", status: "Secure", date: "3d ago", score: 4 },
            ].map((item, i) => (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 font-medium uppercase tracking-tight">{item.type} • {item.date}</p>
                  </div>
                </div>
                <div className="flex items-center gap-8">
                  <div className="hidden sm:block text-right">
                    <div className="text-[10px] font-bold text-slate-300 uppercase tracking-widest leading-none mb-1">Score</div>
                    <div className={cn(
                      "text-xs font-black",
                      item.score > 70 ? "text-rose-500" : item.score > 30 ? "text-amber-500" : "text-emerald-500"
                    )}>{item.score}</div>
                  </div>
                  <span className={cn(
                    "px-2 py-0.5 rounded text-[10px] font-bold border",
                    item.score > 70 ? "bg-rose-50 text-rose-600 border-rose-100" : 
                    item.score > 30 ? "bg-amber-50 text-amber-600 border-amber-100" : 
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 overflow-hidden relative">
            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Risk Scorecard</h4>
            <div className="flex items-end gap-2 mb-6">
              <span className="text-5xl font-black text-slate-900 tracking-tighter">74</span>
              <span className="text-slate-300 font-bold mb-1">/ 100</span>
            </div>
            <div className="space-y-5">
              <div className="space-y-1.5">
                <div className="flex justify-between text-[11px] font-bold">
                  <span className="text-slate-400 uppercase tracking-wider">Critical Risks</span>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-[11px] font-bold">
                  <span className="text-slate-400 uppercase tracking-wider">Compliance</span>
          ...
          <div className="bg-slate-900 rounded-xl p-6 text-white overflow-hidden relative group">
            ...
            <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-4">BERT Summary</h4>
            <p className="text-sm font-medium text-slate-300 leading-relaxed relative z-10 mb-6">
              Last month's analysis revealed 3 critical liability mismatches across 24 documents.
            </p>
            <button className="w-full py-2.5 bg-white text-slate-900 rounded-lg text-xs font-bold hover:bg-slate-100 transition-colors relative z-10">
